...
async def update_user(id: int,updated_user : UserUpdateRequest, db: AsyncSession = Depends(get_db),current_user: User= Depends(get_current_user)):
    ...
    is_current_user(existing_user,current_user)
...
async def delete_user(id : int,current_user = Depends(authorize(allowed_roles = [Role.ADMIN])), db : AsyncSession = Depends(get_db)):
